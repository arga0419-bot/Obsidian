    ...
    function applySpooferToObj(obj)
        ...
        elseif obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
            function updateImage()
                ...
                if changed and obj.Image ~= "rbxassetid://111387348661729" then
                    obj.Image = "rbxassetid://111387348661729"
                end
            end
...
-- ==================== OBSIDIAN UI SETUP ====================
local repo = "https://raw.githubusercontent.com/arga0419-bot/Obsidian/main/"
...
Library.Scheme.OutlineColor    = Color3.fromRGB(80, 80, 80)
Library.Scheme.FontColor       = Color3.fromRGB(220, 210, 255)

local Window = Library:CreateWindow({
    Title = "Leenz Hub",
    Footer = 'TIKTOK | @leenztzy',
...
UnivGroup:AddButton({
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/GVnM9KMS/raw"))()
        end)
        if success then
...
UnivGroup:AddButton({
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/8tfAHteN/raw"))()
        end)
        if success then
...
ParryGroup:AddButton({
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/c2ahhbXW/raw"))()
        end)
        if success then
...
local function GetPredictedToFPosition(originPos, targetPart, targetCharacter, bulletSpeed)
    ...
    if old then old:Destroy() end
        ...
        -- Hapus GUI lama jika masih ada di CoreGui
    if cg:FindFirstChild("ToFTargetSelector") then
        cg.ToFTargetSelector:Destroy()
    end
    ...
    local frame = Instance.new("Frame")
    frame.Name = "Main"
    frame.Size = UDim2.new(0, 165, 0, 130)
    frame.Position = savedUIPos or UDim2.new(0.5, -82, 0.5, -65)
    ...
    headerLbl.Text = "Target: Killer v"
    ...
    btnContainer.Size = UDim2.new(1, -16, 0, 92)
        ...
        for _, m in ipairs(MODES) do
            ...
            if btn then
                if m.internal == targetMode then
                    ...
                    if headerLbl then
                        headerLbl.Text = "Target: " .. m.display .. " v"
                    end
                else
    ...
    function toggleMinimize()
        ...
        if isMinimized then
            ...
            frame.Size = UDim2.new(0, 165, 0, 24)
        else
            minimizeBtn.Text = "−"
    ...
    -- PENTING: Bersihkan tombol lama di dalam container sebelum membuat yang baru
                ...
                if btn and btn.Parent then
                    ...
                    for _, m in ipairs(MODES) do
                        if m.internal == modeName then
                            ...
                            if isActive and headerLbl then
                                headerLbl.Text = "Target: " .. m.display .. " v"
                            end
                        end
                    end
...
KillerCombatGroup:AddToggle("Killer_InfAbyssal", { 
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/GL2tx8XH/raw"))()
        end)
        if success then
...
KillerCombatGroup:AddButton({
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/3kCTZJi5/raw"))()
        end)
        if success then
            Library:Notify({ Title = "Aim Lock", Description = "Aim Lock berhasil dimuat!", Time = 3 })
        else
...
KillerCombatGroup:AddButton({
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/YBbphkCz/raw"))()
        end)
        if success then
...
VeilGroup:AddButton({
    ...
    Func = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/eEeZoNpM/raw"))()
        end)
        if success then
            Library:Notify({ Title = "Silent aim v3", Description = "Silent Veil V3 berhasil dimuat!", Time = 3 })
        else
            Library:Notify({ Title = "Error", Description = "Gagal memuat Silent Veil V3!", Time = 3 })
        end
    end
})
